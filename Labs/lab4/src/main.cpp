...
int main() {
  ...
  while (!game_over) {
    ...
    if (user->cooldown() == 0) {
      ...
      std::println("[2] Применить особое умение (осталось раундов: {})",
                   user->cooldown());
    }
    std::println();
    ...
    switch (user_choice) {
    case (1): {
      std::println("Вы нанесли {:.1f} урона.",
                   bot->take_damage(bot->calculate_damage(user->damage())));
    ...
    case (1): {
      std::println("Противник нанес {:.1f} урона.",
                   user->take_damage(user->calculate_damage(bot->damage())));
